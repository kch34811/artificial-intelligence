...
class ExactInference(object):
    ...
    def observe(self, agentX: int, agentY: int, observedDist: float) -> None:
        ...
        for row in range(self.belief.getNumRows()):
            for col in range(self.belief.getNumCols()):
                ...
                pdfValue = util.pdf(trueDistance, Const.SONAR_STD, observedDist)
                self.belief.setProb(row, col, self.belief.getProb(row, col) * pdfValue)
    ...
    def elapseTime(self) -> None:
        ...
        for (oldTile, newTile), transProb in self.transProb.items():
            ...
            newBelief.addProb(newRow, newCol, self.belief.getProb(oldRow, oldCol) * transProb)
        newBelief.normalize()
...
class ParticleFilter(object):
    ...
    def observe(self, agentX: int, agentY: int, observedDist: float) -> None:
        ...
        for (row, col), count in self.particles.items():
            ...
            trueDistance = math.sqrt((agentX - tileX) ** 2 + (agentY - tileY) ** 2)
            pdfValue = util.pdf(trueDistance, Const.SONAR_STD, observedDist)
            weights[(row, col)] += pdfValue * count
